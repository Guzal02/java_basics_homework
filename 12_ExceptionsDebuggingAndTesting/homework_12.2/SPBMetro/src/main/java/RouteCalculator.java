...
public class RouteCalculator {
    ...
    private List<Station> getRouteWithOneConnection(Station from, Station to) {
        List<Station> fromConnected = from.getLine().getStations();
        List<Station> toConnected = to.getLine().getStations();

        boolean b = fromConnected.stream().anyMatch(station -> {
            Set<Station> connectedStations = stationIndex.getConnectedStations(station);
            if (!connectedStations.isEmpty()) {
                for (Station s : connectedStations) {
                    if (toConnected.contains(s)) {
                        return true;
                    }
                }
            }
